...
class CkCrypt2W : public CkWideCharBase
{
    ...
    public:
	CkCrypt2W();
	CkCrypt2W(CkCrypt2 *p, bool bForDso);
	CkCrypt2W(bool bForDso);
	virtual ~CkCrypt2W();
	...
	CkCrypt2Progress *get_EventCallbackObjectA(void) const;
	void put_EventCallbackObjectA(CkCrypt2Progress *progress);
...
void put_HavalRounds(int propVal);
...
void put_KeyLength(int propVal);
...
void put_PaddingScheme(int propVal);
...
void put_Rc2EffectiveKeyLength(int propVal);
...
const wchar_t *encodedIV(const wchar_t *w1);
const wchar_t *encodedKey(const wchar_t *w1);
const wchar_t *encodedSalt(const wchar_t *w1);

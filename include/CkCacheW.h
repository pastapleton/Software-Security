...
class CkCacheW : public CkWideCharBase
{
    ...
    public:
	CkCacheW();
	CkCacheW(CkCache *p, bool bForDso);
	CkCacheW(bool bForDso);
	virtual ~CkCacheW();
...
const wchar_t *etag(const wchar_t *w1);
const wchar_t *expirationStr(const wchar_t *w1);
const wchar_t *fetchText(const wchar_t *w1);
const wchar_t *filename(const wchar_t *w1);

...
#endif
HCkZipEntry CkZip_InsertNew(HCkZip cHandle, const char *pathInZip, int beforeIndex);
...
const char *CkZip_directoryAsXML(HCkZip cHandle);
const char *CkZip_encryptPassword(HCkZip cHandle);
#if defined(CK_SFX_INCLUDED)
const char *CkZip_exeConfigParam(HCkZip cHandle, const char *name);
#endif
#if defined(CK_SFX_INCLUDED)

...
class CkImapW : public CkWideCharBase
{
    ...
    public:
	CkImapW();
	CkImapW(CkImap *p, bool bForDso);
	CkImapW(bool bForDso);
	virtual ~CkImapW();
	...
	CkImapProgress *get_EventCallbackObjectA(void) const;
	void put_EventCallbackObjectA(CkImapProgress *progress);
...
bool Copy(int msgId,bool bUid,const wchar_t *w1);
...
int GetMailFlag(const CkEmailW &email,const wchar_t *w1);
...
bool SendRawCommandB(const wchar_t *w1,CkByteData &outBytes);
...
bool SetFlag(int msgId,bool bUid,const wchar_t *w1,int value);
bool SetFlags(const CkMessageSetW &messageSet,const wchar_t *w1,int value);
bool SetMailFlag(const CkEmailW &email,const wchar_t *w1,int value);
...
bool StoreFlags(int msgId,bool bUid,const wchar_t *w1,int value);
